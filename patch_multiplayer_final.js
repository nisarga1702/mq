...
const NEW_MULTI = `function Multiplayer({ user }) {
  ...
  React.useEffect(() => {
    ...
    // Combined Players List (Socket + Self if missing)
    ...
    <div style={{display:'grid', gridTemplateColumns:'260px 1fr 280px', gap:12, height:'calc(100vh - 100px)', overflow:'hidden'}}>
      <div className="card" style={{padding:12, display:'flex', flexDirection:'column', gap:8}}>
        ...
        {(players.length?players:[{name:user.name,role:myRole}]).map((p,i) => (
          <div key={i} style={{padding:8, background:'var(--bg3)', borderRadius:8, fontSize:12, fontWeight:600}}>{ROLE_ICONS[p.role]||'🩺'} {p.name} <br/><span style={{fontSize:9,color:'var(--text3)',fontWeight:400}}>{p.role}</span></div>
        ))}
        <div style={{flex:1}}/><button className="btn btn-outline btn-sm" onClick={() => setView('lobby')}>← Lobby</button>
      </div>
        ...
        <div className="card" style={{padding:12, background:'rgba(0,0,0,0.8)'}}>
          <div style={{display:'flex', justifyContent:'center', gap:8, marginBottom:10}}>
            {(TOOLS[myRole] || TOOLS.Doctor).map((t,i) => (
              <button key={i} onClick={() => sendAction(t.action)} style={{background:'rgba(255,255,255,0.05)', border:'1px solid #334155', color:'#fff', padding:'8px 12px', borderRadius:10, minWidth:75}}>
...
if (a !== -1 && b !== -1) {
  ...
  console.log('✓ Multiplayer final polish applied!');
}
`;
